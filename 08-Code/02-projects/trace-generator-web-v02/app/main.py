...
app = FastAPI(title="Trace Generator Web v0.2")
app.add_middleware(
    CORSMiddleware,
...
def latest_parameters():
    """Return the v1 bridge contract: two parameter sets with four fields each."""
    ...
    try:
        ...
        if not isinstance(payload, dict):
            raise ValueError("Parameter 文件必须是 JSON object。")
        raw_ai = payload.get("ai_parameters")
        raw_human = payload.get("human_parameters")
        if not isinstance(raw_ai, dict) or not isinstance(raw_human, dict):
            raise ValueError("Parameter 文件必须同时包含 ai_parameters 和 human_parameters object。")
        ai = engine.validate_parameter_output(raw_ai)
        human = engine.validate_parameter_output(raw_human)
    except Exception as error:
        logger.exception("Latest parameter file invalid: %s", error)
        raise HTTPException(
            status_code=500,
            detail=f"最新 Parameter 文件无效，无法满足四字段 contract：{error}",
        ) from error
    return {"ai_parameters": ai, "human_parameters": human}
