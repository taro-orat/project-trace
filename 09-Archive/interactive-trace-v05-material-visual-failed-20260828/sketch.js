...
const SOURCE_COUNT = 1100;
const AMBIENT_FIELD_MARGIN = 0.03;
...
// v05 Material C: renderer-only visual language. These values never alter
// contribution geometry, alpha policy, or any source lifecycle.
...
function getStableAmbientPosition(
  ...
  // Identity-seeded independent coordinates remove visible rows/columns
  // while keeping every source position stable for the whole session.
  let xUnit =
    getStableAmbientUnit(
      sourceId,
      "x"
    );
  ...
  return {
    x:
      lerp(
        canvasWidth * AMBIENT_FIELD_MARGIN,
        canvasWidth * (1 - AMBIENT_FIELD_MARGIN),
        xUnit
      ),

    y:
      lerp(
        canvasHeight * AMBIENT_FIELD_MARGIN,
        canvasHeight * (1 - AMBIENT_FIELD_MARGIN),
        yUnit
      )
  };
}
...
function drawAccumulationLayers(
  ...
  for (
    ...
    rect(
      -(layerW - baseLayerW) * 0.5,
      -(layerH - baseLayerH) * 0.5,
      layerW,
      layerH
    );
...
function getMaterialSeed(
  ...
  // Deterministic compatibility key for legacy visual pieces. This is not
  // promoted to a source identity and is never used by behaviour code.
  return sourceType
    + ":material:"
    + round(x)
    + ":"
...
function getMaterialColor(
  ...
  return [
    lerp(color[0], luminanceTarget[0], amount),
    lerp(color[1], luminanceTarget[1], amount),
    lerp(color[2], luminanceTarget[2], amount)
  ];
}
...
function drawContributionMaterial(
  ...
  // The base keeps the existing contribution footprint and visual weight.
  fill(
    color[0],
    color[1],
    color[2],
  ...
  if (
    ...
    let edgeVariation =
      getStableUnit(materialSeed + ":edge");

    let topVariation =
      getStableUnit(materialSeed + ":top");

    beginShape();
    vertex(
      materialX,
      materialY + materialH * (0.08 + edgeVariation * 0.08)
    );
    vertex(
      materialX + materialW * (0.72 + topVariation * 0.10),
      materialY
    );
    vertex(
      materialX + materialW,
      materialY + materialH * (0.12 + topVariation * 0.08)
    );
    vertex(
      materialX + materialW * (0.92 - edgeVariation * 0.10),
      materialY + materialH
    );
    vertex(
      materialX + materialW * (0.18 + topVariation * 0.08),
      materialY + materialH * (0.90 - edgeVariation * 0.08)
    );
    endShape(CLOSE);
  ...
  if (
    ...
    let exposure =
      getStableUnit(materialSeed + ":exposure");

    let surfaceShift =
      getStableUnit(materialSeed + ":surface");
    ...
    beginShape();
    vertex(
      materialX + materialW * (0.12 + surfaceShift * 0.08),
      materialY + materialH * 0.14
    );
    vertex(
      materialX + materialW * 0.78,
      materialY + materialH * 0.08
    );
    vertex(
      materialX + materialW * 0.88,
      materialY + materialH * 0.54
    );
    vertex(
      materialX + materialW * 0.34,
      materialY + materialH * 0.66
    );
    endShape(CLOSE);
    ...
    rect(
      round(materialX + materialW * 0.05),
      round(materialY + materialH * 0.08),
      round(max(1, materialW * 0.09)),
      round(max(1, materialH * 0.84))
    );

    let innerX =
      materialX + materialW * (0.10 + exposure * 0.12);

    let innerY =
      materialY + materialH * 0.12;

    let innerW =
      max(1, materialW * (0.34 + exposure * 0.18));

    let innerH =
      max(1, materialH * 0.28);
    ...
    fill(
      ...
      fadedColor[1],
      fadedColor[2],
      alpha * HUMAN_MATERIAL_DETAIL_ALPHA
    );
    ...
    rect(
      round(materialX + materialW * 0.08),
      round(materialY + materialH * 0.72),
      round(max(1, materialW * 0.76)),
      round(max(1, materialH * 0.10))
    );
  ...
  let subdivision =
    getStableUnit(materialSeed + ":subdivision");
  ...
  fill(
    ...
    scanColor[1],
    scanColor[2],
    alpha * AI_MATERIAL_DETAIL_ALPHA
  );

  rect(
    round(materialX + materialW * (0.18 + subdivision * 0.12)),
    round(materialY + materialH * 0.12),
    round(max(1, materialW * 0.18)),
    round(max(1, materialH * 0.76))
  );

  rect(
    round(materialX + materialW * (0.56 + subdivision * 0.12)),
    round(materialY + materialH * 0.14),
    round(max(1, materialW * 0.22)),
    round(max(1, materialH * 0.30))
  );

  rect(
    round(materialX + materialW * (0.56 - subdivision * 0.10)),
    round(materialY + materialH * 0.56),
    round(max(1, materialW * 0.24)),
    round(max(1, materialH * 0.24))
  );
  ...
  rect(
    round(materialX + materialW * 0.08),
    round(materialY + materialH * 0.38),
    round(max(1, materialW * 0.84)),
    round(max(1, materialH * 0.08))
  );

  rect(
    round(materialX + materialW * 0.08),
    round(materialY + materialH * 0.68),
    round(max(1, materialW * 0.84)),
    round(max(1, materialH * 0.06))
  );
}
...
function windowResized() {
  ...
  for (
    ...
    sources[i].humanAmbientPosition =
      getStableAmbientPosition(
        sources[i].humanSource.sourceId,
        width,
        height
      );

    sources[i].aiAmbientPosition =
      getStableAmbientPosition(
        sources[i].aiSource.sourceId,
        width,
        height
      );
  }
