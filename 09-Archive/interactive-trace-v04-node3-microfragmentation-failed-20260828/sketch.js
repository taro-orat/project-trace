...
function getMicroFragmentationState(
  sourceType,
  sourceId
) {
  ...
  return {
    fragmentation,
    pieceCount: constrain(pieceCount, 1, 5)
  };
}


function getMicroFragmentationPieces(
  sourceType,
  sourceId,
  x,
  y,
...
function drawMicroFragmentedContribution(
  sourceType,
  sourceId,
  x,
  y,
        ...
        microPieces:
          entry.state.gatherProgress >= 1
            ? getMicroFragmentationPieces(
                ...
                entry.state.currentPosition.x,
                entry.state.currentPosition.y,
                source.w * capturedVisual.sizeMultiplier,
                source.h * capturedVisual.sizeMultiplier,
                capturedVisual.alpha
              )
            : null
      });
    }
    ...
    contributions.push({
      ...
      microPieces:
        getMicroFragmentationPieces(
          ...
          fragment.x,
          fragment.y,
          fragment.w * capturedVisual.sizeMultiplier,
          fragment.h * capturedVisual.sizeMultiplier,
          capturedVisual.alpha
        )
    });
  }
...
function drawLeavingVisualStates() {
  ...
  for (
    ...
    for (
      ...
      if (
        contribution.microPieces !== null
        &&
        contribution.microPieces !== undefined
      ) {

        for (
          let pieceIndex = 0;
          pieceIndex < contribution.microPieces.length;
          pieceIndex++
        ) {

          let piece =
            contribution.microPieces[pieceIndex];

          push();

          translate(
            piece.x - contribution.x,
            piece.y - contribution.y
          );
  ...
  display() {
    ...
    if (
      ...
      if (
        ...
        drawMicroFragmentedContribution(
          "ai",
          this.aiSource.sourceId,
          aiRenderX,
          aiRenderY,
